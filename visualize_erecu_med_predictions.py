"""Save paper-style EReCu-Med segmentation visualizations.

The script loads an EReCu-Med checkpoint, runs inference on ACDC val/test
volumes, and exports image/GT/prediction/error comparison panels. It is meant
for qualitative figures, not for metric reporting.
"""
...
from EReCu.test_erecu_med_scribble import load_checkpoint, setup_reproducibility
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Comma-separated case prefixes or volume names. Empty means first --max_cases cases.",
    )
...
def error_overlay(image_2d: np.ndarray, pred_2d: np.ndarray, label_2d: np.ndarray) -> np.ndarray:
    ...
    for mask, color in (
        ...
        (wrong_class, colors["wrong_class"]),
    ):
...
def filter_cases(sample_list: Sequence[str], case_ids: str, max_cases: int) -> List[Tuple[int, str]]:
    if case_ids.strip():
        ...
        for idx, name in enumerate(sample_list):
            ...
            if any(stem == item or patient == item or name == item for item in wanted):
                selected.append((idx, name))
        return selected
    return list(enumerate(sample_list[: max(1, int(max_cases))]))
...
def save_summary_grid(
    ...
    if not records:
        return
    n_rows = len(records)
    titles = ["Image", "GT", "Prediction", "Error"]
...
def main() -> None:
    ...
    dataset = BaseDataSets(base_dir=args.root_path, fold=args.fold, split=args.split)
    if len(dataset) == 0:
        raise RuntimeError(f"{args.split} set is empty. Check root_path/fold.")

    cases = filter_cases(dataset.sample_list, args.case_ids, args.max_cases)
    if not cases:
        raise RuntimeError(f"No cases matched --case_ids={args.case_ids!r}.")

    os.makedirs(args.out_dir, exist_ok=True)
    selected_rows = []
    summary_records = []
    print(f"Visualizing split={args.split}, fold={args.fold}, cases={len(cases)}")
    print(f"Checkpoint: {args.checkpoint}")
    print(f"Model key: {metadata.get('resolved_model_key', args.model_key)}")

    for dataset_idx, case_name in cases:
        sample = dataset[dataset_idx]
        image = np.asarray(sample["image"])
        label = np.asarray(sample["label"]).astype(np.uint8)
